...
- [x] **Windows 單一 `.exe`**（2026-08-31，⚠️ 只做過建置驗證，沒有執行驗證）：`./scripts/build.sh win-x64` 會自動加上 `PublishSingleFile`/`IncludeNativeLibrariesForSelfExtract`，跨平台編出一支 71MB 的 `SanringMonitor.exe`（+ 旁邊一個 `wwwroot/` 資料夾，Photino 的相對路徑載入需要它，沒辦法塞進單一檔案裡）。這台是 macOS，只能保證編譯成功，**完全沒人在真實 Windows 上跑過**——`WindowsSecretStore` 的 Credential Manager P/Invoke 是否真的能用、cswap 偵測的 PATH 邏輯在 Windows 上等價的東西是否成立，都還是未知數。順手抓到一個實測才會發現的 bug：純 `<OutputType>Exe</OutputType>` 在 Windows 上會編成 console 子系統，雙擊會多跳一個黑底終端機視窗——已在 `SanringMonitor.csproj` 針對 `win-*` RID 改成 `WinExe`。真的裝成 `.msi`/安裝程式（Start Menu 捷徑、解安裝、之後的自動更新）留到階段 2 再評估值不值得
