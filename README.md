...
- [x] **浮動小工具（widget，2026-08-31，⚠️ 未經人眼視覺驗證）**：`backend/Program.cs` 同時開兩個獨立 `PhotinoWindow`（主視窗 + widget，widget 不是主視窗的 child，關掉主視窗不會連帶關掉 widget）；主視窗的關閉鈕被 `RegisterWindowClosingHandler` 攔下來改成 `SetMinimized(true)`，widget 點「詳細」送 `open-main-window` 就是取消最小化叫回來；整個 app 的存續改掛在 `widgetWindow.WaitForClose()`。widget 是 chromeless+transparent+topmost，收合 80×80 像素風圖示（先用 CSS/SVG 手畫的佔位圖，不是正式美術）、展開 260×180 卡片堆疊，錨在螢幕右下角（展開時 `SetSize`+`SetLocation` 往左上長，不會蓋住其他地方）；卡片順序沿用 `TrackedAccounts` 現有順序，滑鼠拖曳（pointer events，不是 CDK drag-drop——那是為列表排序設計的）左右切換。前端新增獨立的 `WidgetApp` component（跟 `App` 共用 selector `app-root`，`main.ts` 依 `?mode=widget` query param 決定 bootstrap 哪一個，兩者不會同時存在）+ `frontend/src/app/shared/wire-types.ts` 把兩邊共用的 wire 型別抽出來。headless 沒辦法測 GUI，只驗證到「build 過、跑起來不 crash、訊息鏈路有通（log 看得到 widget 視窗自己要到用量資料）」，實際外觀/拖曳手感/位置需要使用者自己看畫面確認
