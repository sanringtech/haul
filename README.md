...
- [x] 主題切換 + 輕量 i18n（2026-08-31）：`theme`/`lang` signal + `effect()` 切 `data-theme` 屬性（沿用 `sanring-theme.css` 本來就有的淺/深色 token）跟查 `frontend/src/app/i18n.ts` 翻譯表，兩者都存 localStorage、按一下當場生效不重載視窗。**刻意不用 `@angular/localize`**——那是 build-time 多 bundle 機制（每語言各編一份完整 Angular 打包），跟這裡要的「app 內即時切換」不合，且會讓 `scripts/build.sh`/C# host 的載入邏輯變複雜。**已知限制**：只涵蓋前端自己寫的 UI 字串（按鈕/標籤/提示），每張卡片的 `detail`/視窗標籤（例如 "5 小時"、"08:07 重置"、各種 provider 錯誤訊息）是後端 C# 產生後直接塞進 `UsageSummary` 欄位的中文字串，語言切換動不到，見 `i18n.ts` 開頭註解
