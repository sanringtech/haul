...
- [x] **浮動小工具（widget，2026-08-31，人眼視覺驗證過大部分功能）**：`backend/Program.cs` 同時開兩個獨立 `PhotinoWindow`（主視窗 + widget，widget 不是主視窗的 child，關掉主視窗不會連帶關掉 widget）；主視窗的關閉鈕被 `RegisterWindowClosingHandler` 攔下來改成 `SetMinimized(true)`，widget 點「詳細」送 `open-main-window` 就是取消最小化叫回來；整個 app 的存續改掛在 `widgetWindow.WaitForClose()`。widget 是 chromeless+transparent+topmost，收合 80×80 像素風圖示（使用者提供的 `pixel-star.svg`）、展開 260×180 卡片堆疊，錨在螢幕右下角（展開時 `SetSize`+`SetLocation` 往左上長，不會蓋住其他地方）；卡片順序沿用 `TrackedAccounts` 現有順序，滑鼠拖曳（pointer events，不是 CDK drag-drop）上下切換。前端新增獨立的 `WidgetApp` component（跟 `App` 共用 selector `app-root`，`main.ts` 依 `?mode=widget` query param 決定 bootstrap 哪一個）+ `frontend/src/app/shared/wire-types.ts` 共用 wire 型別。實測確認：星星圖示/展開收合/拖曳切卡片/結束整個 app 都正常。**已知限制，未解決**：「詳細」按鈕（叫回主視窗）在這台 macOS arm64 開發機上不可靠，連續試過四種修法（攔截關閉鈕改最小化、補 `SetTopMost` 開關、補 `osascript` 跨 app 搶焦點、每次都無條件開新視窗、用 `host.Invoke()` 丟回 UI 主執行緒）都沒解決，決定先不繼續追，維持最初最單純的 `SetMinimized(false)` 版本——之後真的要解可能得等 Photino.NET 更新版本，或評估換框架時一併處理
...
- [x] M4：取消追蹤 vs 關閉顯示，兩個獨立操作（2026-08-31）：「取消追蹤」（完整刪除本機資料含 Keychain，二次確認）在多帳號重構那次就做好了；「關閉顯示」的後端（`SetVisibility`/`set-visibility` 訊息）其實也早就寫好，這次補的是前端——卡片新增「關閉顯示」按鈕（灰色、可逆、無二次確認，跟紅色「取消追蹤」視覺區隔開），設定頁新增「已隱藏的來源」區塊可以叫回來重新顯示（之前隱藏後完全沒有回頭路，資料還在但 UI 找不到）
- [ ] M5：Windows 上實際跑一次打包驗證（`WindowsSecretStore` 目前只是照文件寫的 P/Invoke，還沒真的在 Windows 上測過）
- [ ] 系統匣圖示（tray icon）：Photino.NET 沒有內建跨平台 tray API，查證過三條路（維持現狀+浮動小工具／換 Tauri／Mac 端整個換原生 Swift 像 `stats`/`eul`），目前選擇維持浮動小工具、tray 擱置，見上一條
