...
| Grok 用量來源（**已查證，2026-08-31；2026-09-03 改走 grok-build 端點**） | 僅支援**訂閱制**：`GrokUsageProvider` 讀本機 `~/.grok/auth.json`，打 grok-build 自己的 `GET cli-chat-proxy.grok.com/v1/billing?format=credits`，`isEstimated: false`。**不** `npx ccusage grok`、不寫回登入檔、不 refresh | 已查證：xAI 公開文件沒有餘額端點，API key 制做不到。流水帳不把 Grok CLI 當 runtime |
| Grok 訂閱制官方端點（Grok Build CLI，**已新增，2026-09-03，⚠️ 未實測**） | 新 provider `GrokUsageProvider`（`SourceId: "grok"`，單一本機 session，跟 Cursor / Kimi 訂閱同一類） | 從開源 `github.com/xai-org/grok-build` 的 `billing.rs` / `auth/{config,model}.rs` 讀出端點、5 個 header、OAuth2 scope key、跳過 `web_login`/`api_key`。本開發機沒有 `grok login` / `~/.grok`，路徑在沒憑證時是 `not_configured`，見 §12 |
...
- [x] ~~Grok 用量查詢技術方案~~ —— **已查證（2026-08-31）**：無官方 API key balance 端點，API key 制不支援。**2026-09-03**：訂閱制改走 grok-build 的 billing 端點，不包 ccusage（見 §3/§5/§9）
- [ ] TODO（**新增，2026-08-31**）：**Kimi 訂閱制需要真實帳號驗證** —— `KimiSubscriptionUsageProvider` 已寫但未測，需要有人申請 Kimi Code 帳號實際跑一次確認端點/欄位對不對（見 §5/§9）
- [ ] TODO（**2026-09-03 改狀態**）：**Grok 訂閱制需要真實帳號驗證** —— `GrokUsageProvider` 已寫但本機無 `~/.grok`，需要有人 `grok login` 後跑一次確認端點/欄位對不對（見 §5）
