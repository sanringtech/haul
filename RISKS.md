...
- **Grok 訂閱制（`GrokUsageProvider`，2026-09-03 新增，⚠️ 本機無 `~/.grok` 未實測）**：打 Grok Build CLI 自己用的 `GET cli-chat-proxy.grok.com/v1/billing?format=credits`，讀 `~/.grok/auth.json`。端點/5 個 header/回應欄位來自開源 `xai-org/grok-build`，不是 ccusage、也不 refresh token。沒登入時是 `not_configured`；第一次有人 `grok login` 後才會知道欄位對不對，失敗時會把原始回應截斷顯示。
